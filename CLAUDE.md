...
**Fetching policy:** every request carries a generic browser User-Agent. It must never name the operator, the repository, a machine, or an account, because a repository one person maintains identifies that person to every host it is sent to. `scripts/sources/http.ts` holds the only User-Agent in the project; nothing else sets a request header.

Crawl delay, per-host serialization, and the per-run request budgets are honored for every source without exception, and a run is a handful of requests at most. A source may carry an explicit per-source override of a robots.txt path rule, which records why in `scripts/sources/registry.ts` and prints on every run that uses it. An override covers path permission only, never rate, and never applies to any source that does not declare it.
